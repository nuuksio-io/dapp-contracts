...
import EnvironmentAct from "./EnvironmentAct.cdc"
...
pub contract NFTStorefront {
    ...
    // Supporter share received
    // share of supporter
    pub event SuporterShareReceievd(supporter: Address, amount: UFix64)

    // Residual supporters share
    pub event residualShares(amount: UFix64)

    // Transaction fees 1%
    pub event transactionFees(amount: UFix64)
    ...
    pub struct SaleCut {
        ...
        // EnvAct vault capability
        pub let envActVault: Capability<&EnvironmentAct.Vault{EnvironmentAct.EnvironmentActVaultPublic}>
        ...
        // initializer
        //
        init(receiver: Capability<&{FungibleToken.Receiver}>, envActVault: Capability<&EnvironmentAct.Vault{EnvironmentAct.EnvironmentActVaultPublic}>, amount: UFix64) {
            self.receiver = receiver
            self.amount = amount
            self.envActVault = envActVault
        }
    }
    ...
    pub resource interface SaleOfferPublic {
        ...
        //
        pub fun accept(envActPayment: @EnvironmentAct.Vault): @NonFungibleToken.NFT
    ...
    pub resource SaleOffer: SaleOfferPublic {
        ...
        pub fun accept(envActPayment: @EnvironmentAct.Vault): @NonFungibleToken.NFT {
            pre {
                ...
                envActPayment.getBalance() == self.details.salePrice: "payment vault does not contain requested price"
            }

            let payment <- envActPayment.withdrawFUSD(amount: envActPayment.getBalance())

            // Make sure the offer cannot be accepted again.
            self.details.setToAccepted()

            // get the token's owner which isthe receiver
            let recipientAccount = getAccount(self.nftProviderCapability.address)

            // Fetch the token to return to the purchaser.
            let nft <-self.nftProviderCapability.borrow()!.withdraw(withdrawID: self.details.nftID)
            let envActToken = (&nft as auth &NonFungibleToken.NFT) as! &EnvironmentAct.NFT 
            ...
            // share payment amount according to the EnvAct tokenemics

            // Get a reference to the recipient's Receiver
            let receiverRef = recipientAccount.getCapability(/public/fusdReceiver)!
            .borrow<&{FungibleToken.Receiver}>()
            ?? panic("Could not borrow receiver reference to the FUSD Vault")

            // Get a reference to the token creator's vault
            let creatorRef = getAccount(envActToken.creator).getCapability(/public/fusdReceiver)!
            .borrow<&{FungibleToken.Receiver}>()
            ?? panic("Could not borrow creator reference to the FUSD Vault")

            // Get a reference to the EnvAct vault's Receiver
            let EnvActVaultRef = recipientAccount.getCapability(EnvironmentAct.VaultPublicPath)!
            ...
            let envActVaultPayment <- payment.withdraw(amount: paymentBalance * 0.1) as! @FUSD.Vault
            let royaltyPayment <- payment.withdraw(amount: paymentBalance * 0.1) as! @FUSD.Vault
            let supportersPayment <- payment.withdraw(amount: paymentBalance * 0.05) as! @FUSD.Vault
            let transactionFee <- payment.withdraw(amount: paymentBalance * 0.01) as! @FUSD.Vault
            
            for key in envActToken.supporters.keys {
                let supporterAddress = envActToken.supporters[key]?.address!
                let walletRef = getAccount(supporterAddress).getCapability(EnvironmentAct.VaultPublicPath)
            .borrow<&{EnvironmentAct.EnvironmentActVaultPublic}>()
                if walletRef != nil {
                    let shareAmount = supportersPayment.balance * 0.15
                    walletRef?.topupFUSD(vault: <-(supportersPayment.withdraw(amount: shareAmount) as! @FUSD.Vault))
                    emit SuporterShareReceievd(supporter: supporterAddress, amount: shareAmount)
                }
            }

            // residual go to the central vault
            emit residualShares(amount: supportersPayment.balance)
            EnvironmentAct.vault.deposit(from: <-supportersPayment)
            
            // Transaction fees
            emit transactionFees(amount: transactionFee.balance)
            EnvironmentAct.vault.deposit(from: <-transactionFee)

            // deposit payments
            receiverRef.deposit(from: <-payment)
            creatorRef.deposit(from: <-royaltyPayment)
            EnvActVaultRef.topupFUSD(vault: <-envActVaultPayment)

            destroy envActPayment

            // deposit to supporters
            // for key in nft.supporters.keys {
            //   let address = nft.supporters[key].address
            //   let supporterRef = recipientAccount.getCapability(/public/fusdReceiver)!
            //     .borrow<&{FungibleToken.Receiver}>()
            // }
    ...
    init () {
        ...
        // let storefront <- self.createStorefront()
        // self.account.save(<-storefront, to: NFTStorefront.StorefrontStoragePath)
        // self.account.link<&NFTStorefront.Storefront{NFTStorefront.StorefrontPublic}>(NFTStorefront.StorefrontPublicPath, target: NFTStorefront.StorefrontStoragePath)
